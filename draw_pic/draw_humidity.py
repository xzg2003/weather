...
def _apply_smart_time_axis(ax, start_time: str, end_time: str, tz=BJ_TZ):
    """
    根据查询区间长度，动态设置 x 轴主刻度/格式，避免重叠。
    说明：使用 ax.locator_params(nbins=10) 作为兜底，不覆盖 locator 类型。
    """
...
def generate_humidity_chart(
    start_time: str,
    end_time: str,
    output_path: str = "static/humidity.png",
    db_level: str = "minute",
):
...
'''
    ...
    humidity = _to_float_array(df["humidity"]).copy()
    humidity = np.where(np.isclose(humidity, -100.0, atol=1e-6), np.nan, humidity)
    ...
    if np.any(np.isfinite(humidity)):
        valid_min = float(np.nanmin(humidity))
        valid_max = float(np.nanmax(humidity))
        ymin = max(0, valid_min - 2)
        ymax = min(100, valid_max + 2)
    ...
    for i in range(len(times_local)):
        ...
        x_px = ax.transData.transform((xdata, float(humidity[i])))[0]
